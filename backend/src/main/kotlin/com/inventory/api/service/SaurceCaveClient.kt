...
/**
 * Server-side POST to saurce Cave (`/cave/route`) when [SoaProperties.saurceUrl] is set.
 * Inject where server flows need commerce, wallet, or portfolio facts (same pattern as [ResaurceCaveClient]).
 */
@Service
class SaurceCaveClient(
    ...
    fun postCaveRoute(body: Map<String, Any>): JsonNode? {
        val base = soa.saurceUrl.trim().trimEnd('/')
