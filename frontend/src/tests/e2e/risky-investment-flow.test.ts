...
describe('Risky Investment Flow E2E Test', () => {
  ...
  test('Complete risky investment flow from creation to fallout', async () => {
    ...
    await walletService.investHold(walletId, itemId, 'shipping', amountAtRisk);
    ...
    await walletService.investHold(walletId, itemId, 'additional', 35.00);
    ...
    await walletService.investHold(walletId, itemId, 'insurance', 15.00);
    ...
    const shippingCost = 12.50; // 25 / 2
    const insuranceCost = 7.50; // 15 / 2
    ...
    const borrowerShare = (shippingCost + insuranceCost) / 2;
    const ownerShare = (shippingCost + insuranceCost) / 2;
    const investmentLoss = totalLoss - (shippingCost + insuranceCost);
    
    expect(borrowerShare).toBe(10.00);
    expect(ownerShare).toBe(10.00);
    expect(investmentLoss).toBe(55.00);
  ...
  test('Complete hold lifecycle management', async () => {
    ...
    await walletService.investHold(walletId, itemId, 'shipping', amountAtRisk);
    await walletService.investHold(walletId, itemId, 'additional', 45.00);

    // Simulate shipping to make insurance holds investable
    await shippingService.processItemShipping(itemId, 'TRK345678');
    await walletService.investHold(walletId, itemId, 'insurance', 20.00);
